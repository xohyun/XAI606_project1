...
def write_csv(path, arr):
    ...
    table.loc['ACC(%)'] = arr
    table = table.apply(lambda x: round(x * 100, 2))
    print(table)
    table.to_csv(path)
...
def pretrained_model(save_path, s_dict='model_state_dict', classifier=False):
    args = get_args(save_path)
    print(f"S{args['subject']:02} is loaded.")
    if classifier:
        model = simsiam.classifier()
    else:
        try:
    ...
    model = load_model(model, save_path, s_dict)
    return model.cuda()

def load_model(model, path, s_dict, load_range='all'):
    checkpoint = torch.load(path, map_location='cpu')
    if next(iter(checkpoint[s_dict].keys())).startswith('module'):
        new_state_dict = dict()
        for k, v in checkpoint[s_dict].items():
            new_key = k[7:]
            new_state_dict[new_key] = v
        model.load_state_dict(new_state_dict, strict=True)
    else:
        if load_range == 'all':
            target_module = set(map(lambda x: x.split(".")[0], checkpoint[s_dict].keys()))
            print(f"Target module: {target_module}")
            model.load_state_dict(checkpoint[s_dict], strict=True)
        else:
            target_params = {k: v for k, v in checkpoint[s_dict].items() if
