...
class Args:
    ...
    def make_args(self):
        ...
        # Train
        parser.add_argument('--criterion', default='ND', help="Please enter loss function you want to use.") #CEE
        ...
        parser.add_argument('--learning_rate', '-lr', dest='lr', type=float)
    ...
    def preprocess(self):
        if self.args.mode == 'train':
            ...
            self.set_load_path()
            keys = ['mode', 'get_prediction', 'evaluation', 'summary', 'load_path', 'device']
    ...
    def set_load_path(self):
        if self.args.all_subject:
            if not hasattr(self.args, 'base_load_path'):
                self.args.base_load_path = self.args.load_path
            self.args.load_path = os.path.join(self.args.base_load_path, str(self.args.subject))

    def make_test_args(self, keys):
        args = {key: getattr(self.args, key) for key in keys}
        self.args.update(read_json(os.path.join(self.args.load_path, 'args.json')))
