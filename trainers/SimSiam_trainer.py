...
class Trainer(BaseTrainer):
    ...
    def train_epoch(self):
        ...
        progress = ProgressMeter(
            len(self.data.train_loader),
            [batch_time, data_time, losses],
            prefix="Epoch: []")
            
        for i, data in enumerate(self.data.train_loader):
            # Update model per mini-batch
            input1, input2 = data[0].unsqueeze(1).cuda(), data[1].unsqueeze(1).cuda()
            p1, p2, z1, z2 = self.model(input1, input2)
            ...
            log = self.calculator.calculate(self.args.metrics, loss, numpy=False, acc_count=True)
    ...
    def get_prediction(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.data.test_loader):
                # Get output per mini-batch
                inputs = data.to(self.model.device)
                outputs = self.model(inputs)
